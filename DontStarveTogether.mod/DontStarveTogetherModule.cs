...
public sealed class DontStarveTogetherModule : IGameServerModule, IManifestBackedModule, IModuleConsoleCommandCapability, IModuleExistingServerImportCapability, IModuleGracefulStopCapability, IModulePortCapability, IModuleReadinessCapability
{
    ...
    public ModuleCapabilities Capabilities => Manifest.ToCapabilities(supportsQuery: false, supportsRcon: false) with { SupportsConsoleCommands = true };
    ...
    public Task<IReadOnlyList<ReadinessCheckResult>> CheckReadinessAsync(ServerInstance instance, CancellationToken cancellationToken)
    {
        ...
        checks.Add(File.Exists(executablePath)
            ? ReadinessCheckResult.Pass("DST executable", $"Found: {executablePath}")
            : ReadinessCheckResult.Fail("DST executable", $"Missing {Runtime.StartPath}. Run install/update with SteamCMD app {SteamInstall?.AppId}."));

        var tokenPath = GetClusterTokenPath(instance);
        checks.Add(File.Exists(tokenPath) && !string.IsNullOrWhiteSpace(File.ReadAllText(tokenPath))
            ? ReadinessCheckResult.Pass("Cluster token", $"A cluster token is present at {tokenPath}.")
            : ReadinessCheckResult.Fail("Cluster token", $"Create a Klei cluster token and save it as {tokenPath}."));

        var clusterDirectory = GetClusterDirectory(instance);
        checks.Add(Directory.Exists(clusterDirectory)
            ? ReadinessCheckResult.Pass("Cluster profile", $"Found: {clusterDirectory}")
            : ReadinessCheckResult.Info("Cluster profile", "Master and optional Caves configuration will be created before start."));
    ...
    {
        return ModuleStopStrategyRunner.StopAsync(this, Manifest, instance, cancellationToken, allowKill: false);
    }
